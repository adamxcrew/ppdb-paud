...
class Menu extends CI_Controller {
	...
	public function wali_anak(){
		$data['mSiswa'] = $data['mWali'] = true;
		if($this->session->userdata('level')=='Admin'){
			$data['ortu'] = $this->wali_m->getAll();
		}else{
			$data['ortu'] = $this->wali_m->getByLogin($this->session->userdata('id'));
		}
		$data['content'] = 'siswa/wali';
		$this->load->view('index',$data);
	}
	...
	public function pembayaran(){
		$data['mPembayaran'] = true;
		if($this->session->userdata('level')=='Admin'){
			$siswa = $this->biodata_m->getAll();
		}else{
			$siswa = $this->biodata_m->getByOrtu(_ortu($this->session->userdata('id'))['idorang_tua']);
		}
		$data['all_bayar'] = $this->biodata_m->getBayar();
		$data['siswa'] = ['' => 'Pilih Siswa'] + array_column($siswa, 'nama_lengkap', 'idsiswa');
