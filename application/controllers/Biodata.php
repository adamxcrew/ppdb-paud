...
class Biodata extends CI_Controller {
	...
	public function create(){
		...
		if($this->session->userdata('level')=='Admin'){
			...
			$wali = $this->wali_m->getAll();
		}else{
			$ortu = $this->orangtua_m->getByLogin($this->session->userdata('id'));
			$wali = $this->wali_m->getByLogin($this->session->userdata('id'));
		}
		$data['ortu'] = ['' => 'Pilih Orang Tua'] + array_column($ortu, 'ayah_nama', 'idorang_tua');
		$data['wali'] = ['' => 'Pilih Wali'] + array_column($wali, 'wali_nama', 'idwali');
	...
	{
		if($this->input->post('bayar_tunai',true)==TRUE){
			$idsiswa = $this->input->post('siswa_id',true);
			$data = [
				'siswa_id'=>$idsiswa,
				'keterangan'=>$this->input->post('keterangan',true),
			...
			$this->biodata_m->addBayar($data);
			$this->session->set_flashdata('success','Anda berhasil menambah Data Pembayaran');
		}else{
			$config['upload_path']          = './uploads/';
			$config['allowed_types']        = 'jpeg|jpg|png|bmp';
			$config['max_size']             = 2048;
		...
		redirect(base_url('menu/pembayaran'));
	}
	...
	public function del_bayar($id){
		$del = unlink('./uploads/'.namaBukti($id));
		if($del){
			$this->biodata_m->hapusBayar($id);
			$this->session->set_flashdata('success','Anda berhasil menghapus Pembayaran');
		}else{
			$this->session->set_flashdata('error','Maaf, data gagal dihapus, data terikat dengan data lainnya !');
		}
